...
TEST_F(EepromStm32Test, TestReadByte) {
    ...
    FlashBuf[LOG_BASE+1] = 0x80 | 3;
    /* Write Log word value */
    *(uint16_t*)&FlashBuf[LOG_BASE+2] = (EEPROM_SIZE-2) >> 1;
...
TEST_F(EepromStm32Test, TestWriteByte) {
    ...
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE], 0x8000 | (3 << 8) | 0xbe);
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+2], (EEPROM_SIZE-1) >> 1);
...
TEST_F(EepromStm32Test, TestReadWord) {
    ...
    *(uint16_t*)&FlashBuf[LOG_BASE]   = 0x4000 | (200 >> 1);
    /* Write Log word one-encoded */
    *(uint16_t*)&FlashBuf[LOG_BASE+2] = 0x2000 | ((EEPROM_SIZE-4) >> 1);
    /* Write Log word value */
    *(uint16_t*)&FlashBuf[LOG_BASE+4] = (EEPROM_SIZE-2) >> 1;
...
TEST_F(EepromStm32Test, TestWriteWord) {
    ...
    /* Write Log word zero-encoded */
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE],    0x4000 | ((EEPROM_SIZE-4) >> 1));
    /* Write Log word one-encoded */
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+2],  0x2000 | ((EEPROM_SIZE-2) >> 1));
    /* Write Log word value aligned */
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+4],  200 >> 1);
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+6],  (uint16_t)~0x4321);
    /* Write Log word value unaligned */
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+8],  202 >> 1);
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+10], (uint16_t)~0x763c);
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+12], 202 >> 1);
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+14], (uint16_t)~0xef3c);
    EXPECT_EQ(*(uint16_t*)&FlashBuf[LOG_BASE+16], 204 >> 1);
