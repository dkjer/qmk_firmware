 ...
 * │............│[BYTE][BYTE]│
 * │FFFF....FFFF│[WRD0][WRD1]│
 * │FFFFFFFFFFFF│[WORD][VALU]│
 * │FFFFFFFFFFFF│[BYTE][BYTE]│
 * ├────────────┼────────────┤
 * └──PAGE_BASE │            │
 *    PAGE_LAST─┴─WRITE_BASE │
 *                WRITE_LAST ┘
 *
 * Compacted contents are negated.
 ...
 *
 * ╔═══════════ Word-Entry ══════════╗
 * ║000XXXXXXXXXXXXX║YYYYYYYYYYYYYYYY║
 * ║   └─────┬─────┘║└───────┬──────┘║
 * ║   Address >> 1 ║     ~Value     ║
 * ╚════════════════╩════════════════╝
 *
...
/* These bits are used for optimizing encoding of 0 and 1 */
#define FEE_BYTE_ENCODING  0x8000
#define FEE_VALUE_ENCODED  0x6000
#define FEE_VALUE_ZERO     0x4000
#define FEE_VALUE_ONE      0x2000
...
uint16_t EEPROM_Init(void) {
    ...
    /* Replay write log */
    uint16_t *log_addr;
    for (log_addr = (uint16_t*)FEE_WRITE_LOG_BASE_ADDRESS;
         log_addr < (uint16_t*)FEE_WRITE_LOG_LAST_ADDRESS; ++log_addr) {
        uint16_t address = *log_addr;
        if (address == FEE_EMPTY_WORD) {
            break;
        }
        /* Check for lowest 127-bytes optimization */
        if (address & FEE_BYTE_ENCODING) {
            uint8_t bvalue = (uint8_t)address;
            address = (address >> 8) & 0x7F;
            ...
            if (!(address & FEE_VALUE_ENCODED)) {
                ...
                if ((address & FEE_VALUE_ENCODED) == FEE_VALUE_ENCODED) {
#ifdef DEBUG_EEPROM
                ...
                else if (address & FEE_VALUE_ZERO) {
                    wvalue = 0;
                }
                else /* (address & FEE_VALUE_ONE) */ {
                    wvalue = 1;
                }
            }
            address &= 0x1FFF;
...
static uint8_t eeprom_write_log_byte_entry(uint16_t Address) {
    ...
    /* MSB signifies the lowest 127-byte optimization is in effect */
    uint16_t value = FEE_BYTE_ENCODING | (Address << 8) | DataBuf[Address];
...
uint8_t eeprom_read_byte(const uint8_t *Address) {
    return EEPROM_ReadDataByte((const uintptr_t)Address);
}

void eeprom_write_byte(uint8_t *Address, uint8_t Value) {
    EEPROM_WriteDataByte((uintptr_t)Address, Value);
}
...
uint16_t eeprom_read_word(const uint16_t *Address) {
    return EEPROM_ReadDataWord((const uintptr_t) Address);
}

void eeprom_write_word(uint16_t *Address, uint16_t Value) {
    EEPROM_WriteDataWord((uintptr_t)Address, Value);
}
...
uint32_t eeprom_read_dword(const uint32_t *Address) {
    ...
    if (p % 2) {
        /* Not aligned */
        return (uint32_t) EEPROM_ReadDataByte(p)
             | (uint32_t)(EEPROM_ReadDataWord(p + 1) << 8)
             | (uint32_t)(EEPROM_ReadDataByte(p + 3) << 24);
    }
    else {
        /* Aligned */
